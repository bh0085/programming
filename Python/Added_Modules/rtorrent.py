...
class Transfer(dict):
	...
	def update(self):
		...
		m.d.get_tied_to_file(self['hash'])
		m.d.get_directory(self['hash'])
		m.t.get_url(self['hash'],0)
		...
		self['tied_to_file'] = results[15]
		self['directory'] = results[16]
		self['tracker'] = results[17]
...
#		else:
	...
	def updateStoppedTransfers(self):
		"""Retrieves the latest data about the state of rtorrent and it's transfers."""
		try:
			hashes = self.server.download_list("stopped")
		except xmlrpclib.ProtocolError, e:
			# log the error too
			print "%s %d %s" % (e.url, e.errcode, e.errmsg)
			raise RtorrentFault("An error occured while communicating with the server.")
		...
		stopped = []
		for h in hashes:
			stopped.append(Transfer(h, self.server))
