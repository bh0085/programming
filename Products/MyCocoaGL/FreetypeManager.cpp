...
public:
	FreetypeManager(string font_name="Arial.ttf", int size=50){
		WIDTH = 80;
		HEIGHT = 120;
	...
	unsigned char * getFontAlphaLuminance(unsigned char whichChar){
		slot = face->glyph;
		error = FT_Load_Char( face, whichChar, FT_LOAD_RENDER );
		unsigned char * bitmap = createBMP( &slot->bitmap, slot->bitmap_top,slot->bitmap_left);
		unsigned char * merged = (unsigned char *)calloc(sizeof(unsigned char),2*HEIGHT*WIDTH);

		for(int i = 0 ; i < HEIGHT*WIDTH ;i++){
			merged[i*2] = 255;
			merged[i*2 +1]=bitmap[i];
		}
		free(bitmap);
		return merged;
	}
	unsigned char * getFontRGBA(unsigned char whichChar,float RGBA[4]){
		slot = face->glyph;
		error = FT_Load_Char( face, whichChar, FT_LOAD_RENDER );
		unsigned char * bitmap = createBMP( &slot->bitmap, slot->bitmap_top,slot->bitmap_left);

		unsigned char * merged = (unsigned char *)calloc(sizeof(char),4*HEIGHT*WIDTH);
		for(int i = 0 ; i < HEIGHT*WIDTH ;i++){
			merged[i*4] = (unsigned char)(RGBA[0] *255);
			merged[i*4+1] =(unsigned char)(RGBA[1] *255) ;
			merged[i*4+2] =(unsigned char)(RGBA[2] *255);
			merged[i*4+3]=(unsigned char)(((int)bitmap[i]) * RGBA[3]);
...
extern "C" unsigned char * getFontAlphaLuminance( FreetypeManager * manager, unsigned char whichChar){
	return manager->getFontAlphaLuminance(whichChar);
}
extern "C" unsigned char * getFontRGBA( FreetypeManager * manager, unsigned char whichChar, float RGBA[4]){
	return manager->getFontRGBA(whichChar,RGBA);
}
