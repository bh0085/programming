...
.
"
echo "$var" > "/Users/bh0085/Music/canal/trk-${song}-${artist}"
echo -e "$var"
...
do
    ...
    nextline=`echo "$withnum"|sed -n "$nextlinenum p"`
    nhor=`echo "$nextline"|sed -e "s/^[0-9]*:\([0-9]\{2\}\).*/\1/"                  -e 's/0\([0-9]\)/\1/'`
    nmin=`echo "$nextline"|sed -e "s/^[0-9]*:[0-9]*:\([0-9]\{2\}\).*/\1/"           -e 's/0\([0-9]\)/\1/'`
    nsec=`echo "$nextline"|sed -e "s/^[0-9]*:[0-9]*:[0-9]*:\([0-9]\{2\}\).*/\1/"    -e 's/0\([0-9]\)/\1/'`
    nsecs=$(($nhor*3600 + $nmin*60 + $nsec))    
	...
	num1=`echo "$withnum"|sed -n "$curlinenum s/\(^[0-9]*\).*/\1/p"`
	num2=`echo "$withnum"|sed -n "$(($curlinenum + 1)) s/\(^[0-9]*\).*/\1/p"`
	final=`cat cs|sed -n "$num1,$(($num2-1)) p"`
	starttime=`echo "$final"|sed -n "1 p"`
	song="`echo "$final"|sed -n "2 p"`"
	artist="`echo "$final"|sed -n "3 p"`"
